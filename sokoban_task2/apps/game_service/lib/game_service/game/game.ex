defmodule GameService.Game do
  @moduledoc """
  Game logic for Sokoban - Model in MVC pattern.
  Handles game state as immutable struct and game rules.
  Extended from Task 1 to support database levels.
  """

  defstruct [:board, :player_pos, :width, :height, :won, :goal_positions, :level_id, :moves]
  ...
  @doc "Creates a new game from a level ID"
  @spec new_game(integer()) :: t() | {:error, :level_not_found}
  def new_game(level_id) do
    case Accounts.get_level(level_id) do
      nil ->
        {:error, :level_not_found}

      level ->
        board = Jason.decode!(level.board_json)
  ...
  @doc "Creates a default game for testing"
  @spec new_default_game() :: t()
  def new_default_game do
    # Sample level: player (@), box ($), goal (.), wall (#), empty ( )
    board = [
      "############",
      "#@         #",
      "# $$ ## $$ #",
